  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 bg-gray-50 min-h-screen">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {produits.filter(p => p.nom.toLowerCase().includes(search.toLowerCase())).map(p => (
              ...
              >
                ...
                <p className="text-[10px] text-gray-400 font-bold uppercase">Stock: {p.quantiteStock}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-gray-900 text-white p-6 rounded-[2rem] shadow-xl h-fit sticky top-6">
          <div className="flex justify-between items-center mb-6">
            ...
            {editId && <button onClick={() => {setEditId(null); setPanier([]);}}><X /></button>}
          </div>

          <div className="space-y-4 mb-6 max-h-[300px] overflow-y-auto pr-2">
            {panier.map((item, idx) => (
              <div key={idx} className="flex justify-between items-center bg-gray-800 p-3 rounded-xl">
            ...
            {panier.length === 0 && <p className="text-center text-gray-500 text-xs italic">Panier vide</p>}
          </div>

          <div className="border-t border-gray-800 pt-4">
            <p className="text-[10px] font-black text-gray-500 uppercase">Total</p>
            <p className="text-3xl font-black mb-6">{totalGlobal.toFixed(2)} DH</p>
            <button 
              onClick={handleSubmit}
              disabled={loading || panier.length === 0}
              className="w-full bg-blue-600 hover:bg-blue-500 py-4 rounded-xl font-black transition-all disabled:bg-gray-800 disabled:text-gray-600"
            >
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-black flex items-center gap-2"><History /> DERNIÈRES VENTES</h2>
        <div className="bg-white rounded-2xl border overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y">
              {historique.map((v) => (
                <tr key={v.id} className="text-sm font-bold hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-4 text-right flex justify-end gap-2">
                    <button onClick={() => handleEdit(v)} className="p-2 text-gray-400 hover:text-blue-600 transition-colors"><Edit3 size={18}/></button>
